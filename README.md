...
```
┌─────────────────────────────┐
│  上屏 400×240（不支持触摸） │  状态栏 + 邮件正文 / 列表提示
├─────────────────────────────┤
│  下屏 320×240（电阻触摸屏） │  邮件列表、按钮、设置、软键盘
└─────────────────────────────┘
```
...
| SMTP 发件 | ✅ | 已完成连接 / 登录验证；撰写发送在后续版本开放 |
...
| 内嵌 / 外链图片 | ✅ | CID 内嵌图片与 http 外链图片以 `[图片]` / `[外部链接图片]` 占位，不自动下载外链，避免追踪与证书问题 |
